...
public partial class AotPropertyGrid : UserControl
{
    ...
    public bool ReadOnly
    {
        ...
        set
        {
            ...
            travelChild(pnlProperty, control =>
            {
                ...
                else if (control is Panel listControl)
                {
                    listControl.IsEnabled = !value;
                }
            });
        }
    }

    private void travelChild(Panel panel, Action<Control> action)
    {
        action(panel);
        foreach (Control child in panel.Children)
        {
            if (child is Panel childPanel)
                travelChild(childPanel, action);
        }
    }
    ...
    private Label createPropertyLabel(string propertyName, string propertyDescription)
    {
        ...
        */
        control.PointerPressed += (_, _) =>
        {
            ...
            lblPropertyDescription.Content = propertyDescription;
        };
        return control;
    }
    ...
    private void addPropertyControl(Label label1, Control control1)
    {
        ...
        */
        panel1.Children.Add(control1);
        //panel1.Children.Add(new Splitter() { BackColor = SystemColors.ControlDark, Width = 2 });
    ...
    public void RegisterProperty(string propertyName, string propertyDescription, Func<bool> getValueHandler, Action<bool> setValueHandler)
    {
        ...
        checkBox.IsCheckedChanged += (_, _) => setValueHandler(checkBox.IsChecked.HasValue && checkBox.IsChecked.Value);
        ...
        where TEnum : struct, Enum
    {
        ...
        comboBox.SelectionChanged += (_, _) => setValueHandler((TEnum)comboBox.SelectedItem);
